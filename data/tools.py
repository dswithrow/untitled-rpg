...
class Camera:
    ...
    def update(self, focus):
        x = -focus.rect.x + prepare.SCREEN_SIZE[0]//2
        y = -focus.rect.y + prepare.SCREEN_SIZE[1]//2

        # scroll limiter
        x = min(0, x) # Left
        x = max(-(self.width - prepare.SCREEN_SIZE[0]) , x) # Right
        y = min(0, y) # Top
        y = max(-(self.height - prepare.SCREEN_SIZE[1]), y) # Bottom
